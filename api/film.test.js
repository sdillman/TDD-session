...
test("Film object is expected structure", () => {
    let actual = getFilm()
    let expected = {
    ...
    expect(process.env.ENV).toEqual("dev")
})

test("Film object is expected structure in prod environment", () => {
    let actual = getFilm()
    let expected = {
        "id": "58611129-2dbc-4a81-a72f-77ddfc1b1b49",
        "title": "My Neighbor Totoro",
    ...
    expect(actual).toEqual(expected)
    expect(process.env.ENV).toEqual("prod")
})